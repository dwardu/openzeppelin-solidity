...
class MerkleTreeBuilder {
  ...
  _build (nodes) {
    if (nodes.length === 1) {
      ...
      const midpoint = 2 ** Math.ceil(Math.log2(nodes.length)) / 2;
      const subTreeL = this._build(nodes.slice(0, midpoint));
      const subTreeR = this._build(nodes.slice(midpoint));
...
const MerkleTreeLibrary = new MerkleTreeBuilder({
  hashDataBlock (dataBlock) {
    return keccak256(Buffer.concat([PREFIX_00, dataBlock]));
  },
...
const LegacyMerkleTreeLibrary = new MerkleTreeBuilder({
  hashDataBlock (dataBlock) {
    return keccak256(dataBlock);
  },
  hashNodePair (node1, node2) {
    return keccak256(Buffer.concat([node1, node2].sort(Buffer.compare)));
  },
});
